...
for {set j 0} {$j < 4} {incr j} {
	for {set i 0} {$i < 4} {incr i} {
		$ns duplex-link $n([expr (5*($j)+$i)]) $n([expr (5*($j)+$i+1)]) 1Mb 10ms DropTail
	}
	for {set i 0} {$i < 5} {incr i} {
		$ns duplex-link $n([expr (5*($j)+$i)]) $n([expr (5*($j+1)+$i)]) 1Mb 10ms DropTail
	}
}
for {set i 0} {$i < 4} {incr i} {
	$ns duplex-link $n([expr (4)*(5)+$i]) $n([expr (4)*(5)+$i+1]) 1Mb 10ms DropTail
}
...
$ns rtproto LS
$ns rtmodel-at 10.0 down $n(0) $n(5) 
$ns rtmodel-at 30.0 down $n(7) $n(6)
$ns rtmodel-at 50.0 up $n(0) $n(5)
